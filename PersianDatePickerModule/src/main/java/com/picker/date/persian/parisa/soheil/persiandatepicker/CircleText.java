...
public class CircleText extends TextView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        Log.e(TAG, "onDraw: " + radius + " .... " + (getPaddingRight() / getResources().getDisplayMetrics().density) );

        if (is_animate) {
                paintCircleFill.setColor(color);
                canvas.drawCircle(getWidth() / 2, getHeight() / 2, radius_animate - (getPaddingRight() / getResources().getDisplayMetrics().density), paintCircleFill);
            } else {

                if (radius != 0 && color != 0) {
                    paintCircleStroke.setColor(color);
                    canvas.drawCircle(getWidth() / 2, getHeight() / 2, radius - (getPaddingRight() / getResources().getDisplayMetrics().density), paintCircleStroke);
